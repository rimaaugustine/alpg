...
class DeviceFridge(Device):
    ...
    def simulate(self, timeintervals):
        ...
        for t in range(0, timeintervals):
            ...
            if self.CycleProgress == (self.Runtime+self.Offtime):
                self.State = 1
                self.RuntimeCycle = random.randint(
                    self.Runtime-2, self.Runtime+2)
                self.OfftimeCycle = random.randint(
                    self.Runtime-2, self.Runtime+2)
...
class DeviceKettle(Device):
    def simulate(self, timeintervals, occupancy):
        ...
        # m is time
        m = 0
        while occupancy[m] == 0:  # person occupancy at the index of m, if equal 0
            m += 1  # m = m + 1
    ...
    def simulate(self, timeintervals, occupancy, timestamp):
        ...
        for m in range(0, 1440):
            if (occupancy[m] == 0 or LightingOnProfile[m] == 0):
                ...
                LightingProfile[m] = LightingOnProfile[m] + \
                    ((occupancy[m] - 1)*0.2)
                        ...
                        if dayOfWeek in p.Workdays:
                            if m > p.WorkdayArrival_Avg:
                                FanProfile[m] = FanProfile[m] + \
                                    self.Consumption * len(persons)
                            elif m > p.WeekendBedTime_Avg and m > random.randint(2*60, 3*60):
                                FanProfile[m] = FanProfile[m] + \
                                    self.Consumption
                            else:
                                FanProfile[m] = FanProfile[m]
...
class DeviceWaterPump(Device):
    def simulate(self, timeintervals, day, occupancy, persons):
        ...
        WaterPumpDuration = random.randint(50, 75)
        dayOfWeek = day % 7
        pump = False
        for m in range(0, timeintervals):
            at_home = False
            for idx, p in enumerate(persons):
                if occupancy[idx][m]:
                    ...
                    if at_home:
                        if dayOfWeek in p.Workdays:
                            if m > random.randint(17,21)*60:
                                # print(m)
                                for i in range(m,min(m+WaterPumpDuration,timeintervals)):
                                    WaterPumpProfile[i] = WaterPumpProfile[i] + \
                                        self.Consumption
                                    pump = True
                                if pump:
                                    break;
                            else:
                                WaterPumpProfile[m] = WaterPumpProfile[m]
                        else:
                            if m > random.randint(8,15)*60:  # 5
                                # print(m)
                                for i in range(m,min(m+WaterPumpDuration,timeintervals)):
                                    WaterPumpProfile[i] = WaterPumpProfile[i] + \
                                        self.Consumption
...
class DeviceVentilation(Device):
    ...
    def simulate(self, timeintervals, ventilation):
        ...
        for i in range(0, len(ventilation.VentilationProfile)):
            VentilationProfile.append(int(
                (ventilation.VentilationProfile[i]/ventilation.MaxAirflow) * self.Consumption))
...
class DeviceSolarPanel(Device):
    def simulate(self, startday, timeintervals, pvArea, pvEfficiency, pvAzimuth, pvElevation):
        ...
        while(time < (startday*24*60*60)+timeintervals*60):
            modeltime = int(time - (time % config.weather_timebaseDataset) +
                            int(config.weather_timebaseDataset / 2))
            ...
            index = max(index, 0)
            try:
                irradiation = float(linecache.getline(
                    config.weather_irradiation, index+1))
            except:
            ...
            if elevation > 1 and math.sin(math.radians(elevation)) > 0.25:
                DNI = min(1367.0, (irradiationBeam * 1 /
                ...
                DNI = min(2*irradiationBeam, (irradiationBeam *
            ...
            if GHI < 0.001 or elevation <= 1:
                # No power (significant) irradiation, avoid division by 0.
                pvProfile.append(0)
            else:
                ...
                Gdfs = DHI * (
                    ...
                    (1 + factorF * math.pow(math.cos(math.radians(planeIncidence)), 2) * math.pow(
                        math.sin(math.radians(zenith)), 3))
                )
    ...
    def simulate(self, timeintervals, day, occupancy, washingMoment):
        ...
        if(len(self.EndTimes) > 0):
            if((washingtimeintervals + 1440*(day)) > self.EndTimes[len(self.EndTimes)-1]):
                ...
                washingtimeintervals = (
                    self.EndTimes[len(self.EndTimes)-1] % 1440)+60
        else:
            self.StartTimes.append(washingtimeintervals + (1440*(day)))
...
class DeviceElectricalVehicle(BufferTimeshiftableDevice):
    def simulate(self, day, person, eventStart, eventDuration):
        ...
        if dayOfWeek in person.Workdays:
            ...
            energyLoss = round(
                person.DistanceToWork / (5+(random.randint(0, 100)/100))) * 1000 * 2  # Round trip

            if(random.randint(1, 10) < 3):
                # add a random trip:
                self.StartTimes.append(
                    1440*day + person.WorkdayArrival_Avg + random.randint(150, 210))
                energyLoss = energyLoss + \
                    round(random.randint(5, 20) /
                          (5+(random.randint(0, 100)/100))) * 1000 * 2
            else:
                self.StartTimes.append(
                    1440*day + person.WorkdayArrival_Avg + random.randint(0, 30))

            # approx 25% less range in winter! Not considering heating here
            energyLoss = round(
                energyLoss + (energyLoss * 0.166 * ((math.cos((day/365) * 2 * math.pi)))))

            # print(person.DistanceToWork)
            if(energyLoss > self.BufferCapacity):
                energyLoss = self.BufferCapacity
            # Approx 5.5km/kWh for current (PH)EVs, positive ;-)
            ...
            if(len(self.EndTimes) > 0):
                ...
                self.EndTimes.append(
                    1440*(day) + eventStart - random.randint(30, 60))

            self.Setpoint.append(self.BufferCapacity)
            energyLoss = round(random.randint(
                20, 150) / (5+(random.randint(0, 100)/100))) * 1000 * 2  # Round trip
            # approx 25% less range in winter! Not considering heating here
            energyLoss = round(
                energyLoss + (energyLoss * 0.166 * ((math.cos((day/365) * 2 * math.pi)))))

            self.StartTimes.append(
                1440*day + eventStart+eventDuration + random.randint(0, 60))

            # print(person.DistanceToWork)
            if(energyLoss > self.BufferCapacity):
                energyLoss = self.BufferCapacity
            # Approx 5.5km/kWh for current (PH)EVs
            self.EnergyLoss.append(energyLoss)
